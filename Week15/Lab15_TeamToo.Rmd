...
andrewsData <- andrewsData%>%
  ...
  filter(andrewsData$`Temperature(F)`>= -40 & andrewsData$`Temperature(F)`<= 120)%>%
...
predsTemp <- andrewsData %>%
  data_grid(`Temperature(F)` = seq_range(`Temperature(F)`, 2180995))%>%
...
By investigating where serious accidents occur, we can attempt to find roads/regions where efforts to improve road safety should be improved.
