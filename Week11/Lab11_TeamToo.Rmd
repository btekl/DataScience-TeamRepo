...
```
Baker name info -> We can fill in if the name works

```{r}
anal_baby_2018 <- babys%>%
  filter(sex == "M", year == "2018", str_length(name) < 6)%>%
  arrange(desc(prop))

anal_baby_2017 <- babys%>%
  filter(sex == "M", year == "2017", str_length(name) < 6)%>%
  arrange(desc(prop))

anal_baby <- anal_baby_2017%>%
  full_join(anal_baby_2018, by = "name")%>%
