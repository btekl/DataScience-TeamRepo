...
all_names_before <- babys %>%
  filter(sex == 'M', year == 1995) %>%
  full_join(all_names1, by = "name") %>% 
  replace_na(list(prop.x=0, prop.y=0)) %>%
  mutate(prop_change = (prop.x-prop.y))
all_names2 <- babys %>%
  filter(sex == 'M', year == 1995)
all_names_after <- babys %>%
  filter(sex == 'M', year == 2018) %>%
  full_join(all_names2, by = "name") %>% 
  replace_na(list(prop.x=0, prop.y=0)) %>%
  mutate(prop_change = (prop.x-prop.y))
```
...
```{r}
(change_brian_before <- all_names_before %>%
  filter(str_detect(name,"^Br[iy][oa]n")) %>%
  summarise(change_brian = sum(prop_change)))
percentile_before <- ecdf(all_names_before$prop_change)
percentile_before(change_brian_before)
```

```{r}
(change_brian_after <- all_names_after %>%
  filter(str_detect(name,"^Br[iy][oa]n")) %>%
  summarise(change_brian = sum(prop_change)))
percentile_before <- ecdf(all_names_after$prop_change)
percentile_before(change_brian_after)
```
