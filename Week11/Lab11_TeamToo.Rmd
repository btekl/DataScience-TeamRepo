...
The Popularity of Clint names peaked around 1910 and had another smaller peak around the 1980s and has steadily declined to 2018. I think that this second peak can be explained by the popularity of actor Clint Eastwood as corresponding to the decline of his influence as an actor the proportion of Cli names steadily decreases following 1980. 
...
We created a pair of functions to make these calculations easier. The first function takes two years and a regex as an argument, and returns the change in proportion from the first year to the second. The second function takes a pair of years, and calculates the change in proportion for every name in the dataset between those two years. This set of data can then be passed to the r function ecdf, to build the distribution of proportion changes for that set of years. Then, our individual changes are compared to the distribution to get a percentile result.
```{r}
...
actual change of names 1976-1997
```{r}
get_prop_change(1976,1997,"^And[y|rew]")
```

relative change names 1976-1997
```{r}
percentile_before <- ecdf(get_allnames_prop_change(1976,1997)$prop_change)
percentile_before(get_prop_change(1976,1997,"^And[y|rew]"))
```

actual change 1997-2018
```{r}
percentile_before <- ecdf(get_allnames_prop_change(1997,2018)$prop_change)
percentile_before(get_prop_change(1997,2018,"^And[y|rew]"))
```
...
actual change of names 1999-2018
```{r}
get_prop_change(1999,2018,"^Charl[ie][sey]")
```

relative change names 1999-2018
```{r}
percentile_before <- ecdf(get_allnames_prop_change(1999,2018)$prop_change)
percentile_before(get_prop_change(1999,2018,"^Charl[ie][sey]"))
```

actual change 1980-1999
```{r}
get_prop_change(1980,1999,"^Charl[ie][sey]")
```

relative 1980-1999
```{r}
percentile_before <- ecdf(get_allnames_prop_change(1980,1999)$prop_change)
percentile_before(get_prop_change(1980,1999,"^Charl[ie][sey]"))
```

#### Clint
actual change in Clint from 1979 to 1999:
```{r}
get_prop_change(1979,1999,"^Clint")
```
The relative change:
```{r}
percentile_before <- ecdf(get_allnames_prop_change(1979,1999)$prop_change)
percentile_before(get_prop_change(1979,1999,"^Clint"))
```
Clint names experienced a major decrease in popularity

actual change in Clint names from 1999 to 2018:
```{r}
...
```{r}
percentile_before <- ecdf(get_allnames_prop_change(1999,2018)$prop_change)
percentile_before(get_prop_change(1999,2018,"^Clint"))
```
The popularity of Clint names continued to decline but as they were already a smaller proportion of total names that decline was lesser.
...
For a female name, we recommend Lyra. Lyra only appears as a female name in the dataset, and doesn't easily shorten to a nickname ("Ly" does appear in the dataset, but with a maximum count of 8 since 2000). In terms of total number, Lyra has been trending upward steadily since 2000. Proportion has increased similarly, though with a slight dip in 2018.

```{r}
ggplot(filter(babys,name == "Lyra",year >1999))+
  geom_line(aes(year,n))+
  xlab("Year")+
  ylab("Number of Female Babies")+
  ggtitle("Number of Female Babies Named Lyra by Year")
```

For a male name, we recommend Baker. It can't be shortened into a nickname, though it does have some split on sex, with around 5-10% of Bakers in a given year being female. The overall popularity of Baker has begun to increase substantially in recent years:
```{r}
baker <- babys %>%
  filter(str_detect(name, "^Baker$"), babys$sex == "M")

ggplot(baker) +
  geom_line(aes(year, n, color = "Number of Children named Baker")) +
  geom_vline(xintercept = 2010, color = "2010") +
  scale_color_manual(name="" ,values=c(`Number of Children named Baker`='black', `2010`= 'red')) +
  xlab("Year") +
  ylab("Number of Male Babies named Baker")+
  ggtitle("Change in Number of All Babies Named Baker Over Time")
```

### Individual Contributions

**Andrew:**
I am currently dying from the flu but attempted to find baby names using i set of string detects and found the popularity of those names.

**Brian:** I found out how to use manual scales to add a legend manually to a plot. I also found the ecdf function, to find a percentile given a vector of data.

**Charles:**
I am not incredibly proud of my contributions this week. I was able to use the tools and determine the popularity of my name though I do wish I was able to figure out the Ariel part earlier so my team mates could have laid back more.

**Clint:**
I used stringr and dplyr to filter the dataset to discern the popularity of my name over time and then used the tidyverse to graph my observations. I also used dplyr and stringr to filter the babys dataset in order to find a male name that would be short (less likely to have an easy nickname), trendy (the proportion of total names increased from 2017-2018), but not trending(outside of the top 200 boy names of 2018). 
