...
**Notable Features:** The tone of a question, formatting, and specificity seem to influence the score of a question. 

Answers which include examples, are of adequate length, and are written with a positive tone, influence the score of a submission.

**Ideal question and answer:** Though our findings were not all conclusive, the ideal question should not be demanding, and properly articulated.

The ideal answer should be specific, with examples, long enough to sufficiently answer the question, and written with a positive tone.
...
My feature which plots the average score for each question using the word "integer". I figured this was a basic concept learned in many intro math classes and it would be interesting to see how well people react to questions with this concept. This feature is created in order to view the importance of this program to people/organization asking the questions. This will help us to analyze how important this concept is in the field.
...
ggplot(meanint, mapping = aes( x= integer,y = mean, fill = integer ))+
  ...
  scale_fill_gradient(low = 'red', high = 'yellow')+
   ggtitle("Mean Score for Questions involving the concept Integer")+
  ylab("Mean Score")+
...
We can see the questions 1- 4 seem to be a lot easier than questions 5 - 7. When looking at the actually questions. It seems that it also took longer for the person to answer the questions 5-7 meaning they were much more difficult. Overall, we can see that many people average well on the easy questions and poor on the hard. This string was commonly used and very useful in the field.

For my answers I decided to check the time it too answer when the person answered with the word failed. It would be interesting to see a correlation between the word and the time the person took to answer.

```{r}
failed <- Q_and_A2%>%
  mutate(failed = str_count(AnswerBody, regex("failed", ignore_case =  TRUE)))

medfailed <- failed %>%
  group_by(failed) %>%
  summarise(mean = mean(time_to_ans,na.rm = TRUE))
  
  

ggplot(medfailed, mapping = aes( x= failed,y = mean/1000000, fill = failed ))+
  geom_col()+
  scale_fill_gradient(low = 'red', high = 'blue')+
   ggtitle("Time it took Answer a Question correlated to the amount of time failed was said")+
  ylab("Time it took to answer")+
  xlab("Question Number")+
  labs(fill = "Number of uses of the word failed")+
  scale_x_discrete(limits = c(1,2,3,4))+
  xlim(0,5)

```

We can see if the person used the word failed once then they look a long time to explain why. It is interesting to see that of they used failed a lot, or 4 time, then they were very quick to answer.

#### Brian

For questions, I created a feature related to the number of mentions of the word "need". I figured questions that came off as demanding would be scored lower overall, and tried features with a variety of words that could come across as demanding, eventually settling on "need".
...
I want to see if a question with a high score is more likely to produce and answer with a high score. I would predict that higher score questions would produce higher scoring answer because I would think that more interesting or insightful questions would be more likely to elicit a strong answer. 

```{r}
### First I want to get an assesment of the basic relationship betweeen question score and answer score
ggplot(Q_and_A2) + geom_smooth(aes(Score_Que, Score_Ans)) + 
  geom_point(aes(Score_Que, Score_Ans)) +
  xlim(0,500) +
  ylim(0,500) 

```

This graph isn't particularly insightful, but maybe by examining the average score at each question score the relationship will become more clear.

```{r}
prep_qanda <- Q_and_A2 %>%
  group_by(Score_Que) %>%
  summarise(mean = mean(Score_Ans))

ggplot(prep_qanda) + geom_smooth(aes(Score_Que, mean)) + 
  geom_point(aes(Score_Que, mean)) +
  xlim(0,500) +
  ylim(0,75) 
```
This graph suggests a much stronger relationship between question score and answer score than I first would have anticipated. At the very low questions scores are corresponingly related to low answer scores in a very strong positive fashion up to a question score of 200, from which the line tapers off. This suggests that higher scoring questions are likely to inspire higher scoring answers up to a point, after which the quality matters less. 

#### Clint
Ideas for feature -questions: I want to see if a question with a high score is more likely to produce and answer with a high score. I would predict that higher score questions would produce higher scoring answer because I would think that more interesting or insightful questions would be more likely to elicit a strong answer. 
...
This graph suggests a much stronger relationship between question score and answer score than I first would have anticipated. At the very low questions scores are corresponingly related to low answer scores in a very strong positive fashion up to a question score of 200, from which the line tapers off. This suggests that higher scoring questions are likely to inspire higher scoring answers up to a point, after which the quality matters less. 
 ...
 I was curious how the length of a answer effects the score? I was also curious to see how the inclusion of code effected the average scores at different answer lengths. I would suspect that longer responses would generally lead to higher scores. 
```{r}

fst_anal<- Ans %>% 
  filter(str_detect(Ans$Body , "<code>") == TRUE)

fst_anal2 <- Ans %>% 
  filter(str_detect(Ans$Body , "<code>") == FALSE)
```
...
```{r}
### actually first I'll see if there appears to be a relationship between length of response and score

secnd_anal2 <- Ans %>%
  mutate(body_leng = str_length(Ans$Body)) %>%
  group_by(body_leng)%>%
   summarise(mean = mean(Score))
fst_anal_b <- fst_anal %>%
  mutate(body_leng = str_length(fst_anal$Body))%>%
  group_by(body_leng)%>%
   summarise(mean = mean(Score))
fst_anal_2b <- fst_anal2 %>%
  mutate(body_leng = str_length(fst_anal2$Body))%>%
  group_by(body_leng)%>%
   summarise(mean = mean(Score))

long_body <- secnd_anal2 %>%
  filter(body_leng > 3000)
short_body <- secnd_anal2  %>%
  filter(body_leng < 900)

ggplot(secnd_anal2, mapping = aes(x= secnd_anal2$body_leng, y = secnd_anal2$mean)) + 
  geom_smooth() +
  geom_point() +
  ggtitle("Effect of Answer Length on Score")+
  ylab("Average Score of the Response")+
  xlab("Length of the Response") + 
  xlim(0,10000) +
  ylim(0,75)

ggplot(fst_anal_b, mapping = aes(x= fst_anal_b$body_leng, y = fst_anal_b$mean)) + 
  geom_point() +
  geom_smooth() +
 xlim(0,10000) +
  ylim(0,75)+
  ggtitle("Effect of Answer Length on Score of Responses with <code>")+
  ylab("Average Score of the Response")+
  xlab("Length of the Response")

ggplot(fst_anal_2b, mapping = aes(x= fst_anal_2b$body_leng, y = fst_anal_2b$mean)) + 
  geom_point() +
  geom_smooth() +
 xlim(0,10000) +
  ylim(0,75) +
  ggtitle("Effect of Answer Length on Score of responses without <code>")+
  ylab("Average Score of the Response")+
  xlab("Length of the Response")
  ggplot(long_body, mapping = aes(x= long_body$body_leng, y = long_body$mean)) + 
  geom_point() +
  geom_smooth() +
 xlim(3000,10000) +
  ylim(0,75) +
  ggtitle("Effect of Long Answer Length on Score of responses without <code>")+
  ylab("Average Score of the Response")+
  xlab("Length of the Response")

ggplot(short_body, mapping = aes(x= short_body$body_leng, y = short_body$mean)) + 
  geom_point() +
  geom_smooth() +
 xlim(0,900) +
  ylim(0,75) +
  ggtitle("Effect of short Answer Length on score")+
  ylab("Average Score of the Response")+
  xlab("Length of the Response")
```
From examining the graphs you can see that the vast majority of responses fall around 1800 characters and unsurprisingly the responses containing code did better, however with response containing no code at all there was a much stronger correlation between length and score. This suggests tht amongst response with no code a longer wordier answer may have been able to achieve a higher score. By splitting the overall graph into graphs examining just short answers, under 900 characters, and long answers, over 3000, I would suggest that there is not a relationship between the length of answer and score in general as the line of best fit if very flat across all three graphs.
