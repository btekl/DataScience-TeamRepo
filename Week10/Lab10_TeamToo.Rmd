...
For my answers I decided to check the median score on answers that involve the string "failed" because i felt it would be interesting to see the correlation between this pratical string and results.
...
medfailed <- failed %>%
  group_by(failed) %>%
  summarize(med = mean(Score_Que,na.rm = TRUE)) 

ggplot(medfailed, mapping = aes( x= failed,y = med, fill = failed ))+
  geom_col()+
  scale_fill_gradient(low = 'red', high = 'yellow')+
   ggtitle("Median Score for Answer involving the string Failed")+
  ylab("Median Score")+
  xlab("Question Number")+
  labs(fill = "Question")+
