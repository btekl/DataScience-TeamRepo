...
#### Clint
Ideas for feature -questions: I want to see if a question with a high score is likely to produce and answer with a high score. Is there are a relationship between question score and answer score? 
...
mean_code <- mean(fst_anal$Score)

mean_para <- mean(fst_anal2$Score)

mean_gen <- mean(Ans$Score)
```
...
mean_length <- mean(secnd_anal$body_leng)

ggplot(secnd_anal, mapping = aes(x= secnd_anal$body_leng, y = secnd_anal$mean)) + 
  geom_smooth() +
  geom_point() +
