...
candy2 %>%
  ...
  arrange(desc(Meh))
```
Lastly, the candies which no one had any opinion on at all:
```{r}
candy2 %>%
  filter(is.na(Rating)) %>%
  group_by(`Candy Type`) %>%
...
```{r}
ggplot(candy3) +
  geom_histogram(aes(Score),bins=79)+
...
Finally, we ran summaries on the various demographics of the dataset, generated the score variable to compare overall preferences among candy types, and some filters to f
