...
**Why this question is interesting:** This would be an interesting question because it would explain which candies are most popular by each country and allow us to compare culture between nations.
...
**Why this question is interesting:** Understanding how preferences vary among different age groups can provide interesting insights. Perhaps some candies are universally popular among all ages, while others appeal only to certain groups.
...
# Malawi
Malawi <- Filter(function(x)!all(is.na(x)), Malawi)

Malawi <- rename(Malawi,ybp1=`Age, ybp`, Age1 = `Age (calender)...2`, BSi1 = `BSi MAR (mgSiO2/cm2y)...3`, ybp2 = `Age, cal ybp`, Age2 = `Age (calender)...7`,BSi2 = `BSi MAR (mgSiO2/cm2y)...8`)

Malawi <- Malawi%>%
  filter(!is.na(BSi1))

Malawi1 <- Malawi %>%
  select(ybp1, Age1, BSi1)
Malawi1$ybp1 <- parse_double(Malawi1$ybp1)
Malawi1$ybp1 <- round(Malawi1$ybp1)
Malawi1$Age1 <- round(Malawi1$Age1)
Malawi1$BSi1 <- round(Malawi1$BSi1,2)

Malawi2 <- Malawi %>%
  select(ybp2, Age2, BSi2)
Malawi2$ybp2 <- parse_double(Malawi2$ybp2)
Malawi2$ybp2 <- round(Malawi2$ybp2)
Malawi2$Age2 <- round(Malawi2$Age2)
Malawi2$BSi2 <- round(Malawi2$BSi2,2)
```
...
ggplot(MalawiAD, mapping = aes(x = Age1, y= BSi1))+
  geom_smooth()+
  xlab("Year AD")+
  ylab("BSI MAR ")+
  ggtitle("Levels of BSI MAR versus Year in AD for Malawi")
```

When looking at how the BSI MAR levels have change since 0 AD, we can notice that since around 1200 AD, the BSI MAR levels of Malawi are increasing drastically as time moves forward. There were no data points from 1847 - 2019 AD so we are unable to analyze how trends are changing since 1847, however; from the results we do have, we would expect the average BSI levels to increase.
...
VOI <- VOI %>%
  select(-`...14`,-`...15`)
ggplot(VOI)+
  geom_smooth(aes(Year,Rainfall,color=Month),se = FALSE)
```

Rainfall is highest in March, April, November, and December across the entire 20th century. There is also a notable shift in the 1950's, when rainfall occurred earlier than in other years. March and November see a rise in recorded rainfall, while April and December see reduced amounts of rainfall. This trend holds for November and December for the rest of the century. In the late 70's/early 80's, the trend reverses for March: rainfall drops considerably, and April again sees higher totals of rainfall.
