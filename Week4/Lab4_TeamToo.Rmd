...
The effects of these differences are difficult to measure. Generally, older mothers have more difficulties during and after labor than younger mothers. Mothers who have had many pregnancies can experience more complications giving birth than those with only a few. However, in our data, we've found a difference of 2 prior pregnancies on average for smokers, versus 1 for non-smokers--both are on the lower end. Finally, educational differences between these groups could also be influencing our conclusions. Studies suggest premature birth rates are lower among mothers with college/trade school education compared to those whose education stopped at high school (see [Clint's Plot]). This discrepancy in education levels presents another potential factor which may bias our conclusions.
...
Notable differences between smokers and non-smokers are the age of the mother (non-smokers are slightly older on average), BMI (see [Brian's Plot]), parity (smokers on average have had one additional pregnancy), and the level of education of both the mother (see [Clint's Plot]) and father (a larger proportion of non-smokers graduated college).

The exact effects of these differences are difficult to gauge, but they present possible unaccounted connections between variables which may be influencing our determination on the effects of smoking.

```{r Team Plot, echo=TRUE, warning=FALSE}
plot3 <- ggplot(data=babies2)
plot3+geom_histogram(aes(ded),bins=8)+facet_wrap(~smoke,labeller=bool_smoke)+xlab("Father's level of Education") 
plot3+geom_boxplot(aes(factor(smoke),mage,group=smoke,middle=mean(mage)))+ylab("Mother's age")+scale_x_discrete(breaks=c("0","1"),
        labels=c("Non-Smokers","Smokers"))+theme(axis.title.x=element_blank())
plot3+geom_boxplot(aes(factor(smoke),parity,group=smoke,middle=mean(parity)))+ylab("Number of prior Pregnancies")+scale_x_discrete(breaks=c("0","1"),
        labels=c("Non-Smokers","Smokers"))+theme(axis.title.x=element_blank())
