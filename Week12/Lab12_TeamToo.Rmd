...
```{r}
data <- gapminder_unfiltered # replace w/ updated dataset
```
...
perm_mean <- function(perms = 1000, all_values, n_A)
{
  ...
  for (i in c(1:perms))
  {
    ...
    samp <- sample(all_values)
    rand1 <- samp[1:n_A]     
    rand2 <- samp[(n_A+1):length(all_values)]    
    # Step 3:
...
This correlation existed in both datasets; however, the unfiltered dataset contains the additional FSU (former Soviet Union) continent, which has a moderate negative correlation (-.59) between population and life expectancy. Oceania has more data as well, and we find a weak positive correlation with similar p-values as above.

This extra data helps us infer that this correlation arises in the continents with fewer countries overall. The most populous countries in each region (Russia, Australia, United States/Brazil) describe a large proportion of their continent's dataset: the correlation that arises between population and life expectancy is heavily weighted by the overall trends of these countries.

### Individual Contributions

**Andrew:**
I helped to create the functions and make sure they were able to use a new data-set. I helped come uo with ideas that were not used.
...
**Charles:** I assisted Brian in his formulation and deployment of solutions for the dataset. I also partook in the translation of our recitation sections code to be applied to the document.

**Clint:**
I came up with some alternate questions, values, and analysis. I tested the functions and I assisted in the implementation of the new dataset into the code. 
